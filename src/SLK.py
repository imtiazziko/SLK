...
def SLK(X,sigma,K,W,bound_= False, method = 'MS', C_init = "kmeans_plus", bound_lambda=1.0, **opt):
    ...
    for i in range(100):
        ...
        if bound_==True:
            ...
            if X.shape[0]>100000:
                batch = True
            if method == 'BO':
                l,C,mode_index,z,bound_E = bound.bound_update(a_p,X,W,bound_lambda,bound_iterations,batch,manual_parallel)
            else:
                l,_,_,z,bound_E = bound.bound_update(a_p,X,W,bound_lambda,bound_iterations,batch,manual_parallel)
